...
- [x] 対象メッセージを取得（companyId + 期間）
- [x] サイズ制御（長文対策）
  - [x] 直近N件/直近N文字に切る
  - [ ] 重要そうなメッセージ（To/返信/特定ラベル）を優先する（拡張）
- [x] 取り込み未完（companyId=null）を除外する

### 2) Backend：LLMインターフェース（AI）

- [x] `LLMClient` インターフェースを定義（`summarize(messages)->draft`）
- [x] 実装：OpenAI（or Azure）を1つ用意
- [ ] タイムアウト/リトライ/エラーメッセージ整形
- [ ] 機微情報（Should）：マスキング余地を残す（簡易でも可）

### 3) Backend：プロンプト/出力形式（AI）

- [x] 出力の必須項目を固定（要件：重要トピック/未解決/次アクション）
- [x] JSONで返すか、Markdownで返すか決める
  - [ ] 推奨：内部はJSON（安定）、UI表示はMarkdown整形
- [x] sourceLinks（引用元）を「messageId配列」として返せるようにする（MVPは粗くてもOK）

### 4) Backend：API実装（AI）

- [x] `POST /companies/:id/summaries/draft`
- [x] `POST /companies/:id/summaries`（保存、manual/auto）
- [x] `GET /companies/:id/summaries`（履歴：新しい順）
- [x] RBAC：閲覧はread、保存はwrite

### 5) Backend：タスク候補抽出（Should / AI）

- [x] draftから「次アクション」を抽出し候補化（まずはルールベースでOK）
- [x] 候補の項目：title / dueDate（推定できれば）/ assignee（推定できれば）
- [x] 候補→`POST /tasks` に変換できる

### 6) Backend tests（AI）

- [x] LLMをモックして要約生成ができる
- [ ] 期間フィルタで対象メッセージが変わる
- [x] 要約履歴が保存され、一覧で取得できる
...
企業詳細（要約ブロック）：
- [x] 期間選択（7日/30日/カスタム）
- [x] 生成→プレビュー（draft）→編集→保存
- [x] 要約履歴（最新＋過去）
- [x] 引用元リンク（タイムラインの該当メッセージへジャンプ）
- [x] タスク候補表示→「タスク化」ボタン
...
- [x] `git add -A`
- [x] `git commit -m \"feat: ai summaries with human review\"`
- [ ] `git push`
