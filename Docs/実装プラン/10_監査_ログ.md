...
- [ ] 記録対象の最小集合を決める（MVP：Company/Task、拡張：Project/Wholesale/User/設定）
- [ ] 差分の形式を決める
  - [ ] MVP案：`changes` に `{ before, after }` をJSON文字列で保存
  - [ ] 将来案：JSON Patch（RFC6902）
- [ ] 重要操作のカテゴリを決める（例：`action=delete`、`entityType=User` のrole変更など）

### 2) Backend：AuditLog書き込み（AI）

- [ ] 共通ヘルパーを作る（例：`audit.log({entityType, entityId, action, userId, changes})`）
- [ ] 既存/今後の各ルートで、成功時に AuditLog を作成
- [ ] `userId` は JWT の `userId` から取得（未ログイン操作はnull）
- [ ] 失敗時は AuditLog を書かない（成功のみ）

### 3) Backend：参照API（AI）

- [ ] `GET /audit-logs`（entityType/entityId/期間/ページング）
- [ ] RBAC：閲覧は admin（最小） or read権限（どちらにするか決める）

### 4) Backend tests（AI）

- [ ] Company更新で AuditLog が1件増える
- [ ] userId が記録される
- [ ] 参照APIで対象のログが返る

### 5) Frontend（AI）

- [ ] 企業詳細に「変更履歴」タブ（Companyの更新ログを表示）
