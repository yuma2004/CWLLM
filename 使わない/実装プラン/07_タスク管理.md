...
- [x] `Task.status` の値を固定（例：`todo`/`in_progress`/`done`/`cancelled`）
- [x] `targetType` の値を固定（`company`/`project`/`wholesale`）
- [ ] 期限（dueDate）のタイムゾーン方針を決める（MVP：UTC保存、表示はローカル）

### 2) Backend：Task CRUD（AI）

- [x] `POST /tasks`
  - [x] 必須：targetType/targetId/title
  - [x] 任意：description/dueDate/assigneeId/status
- [x] `GET /tasks`：フィルタ（status/dueDate範囲/assigneeId/targetType/targetId）
- [x] `PATCH /tasks/:id`：部分更新（status更新は頻出なのでテストを厚め）
- [x] RBAC：readはログイン必須、writeは `requireWriteAccess()`

### 3) Backend：マイタスク（AI）

- [x] `GET /me/tasks`（assigneeId=自分）
- [ ] プリセット（期限超過/今日/今週）をAPIで持つかUIで持つか決める（MVPはUIでOK）

### 4) Backend tests（AI）

- [x] Task作成→企業詳細のタスク一覧に出る
- [x] マイタスクに自分のタスクだけ出る
- [x] ステータス更新が反映される
...
- [x] `git add -A`
- [x] `git commit -m "feat: task management"`
- [ ] `git push`
