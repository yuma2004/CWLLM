...
function Exports() {
  ...
  const [errorScope, setErrorScope] = useState<'companies' | 'tasks' | ''>('')
  ...
  const isCompanyRangeInvalid = Boolean(
    companyFilters.from && companyFilters.to && companyFilters.from > companyFilters.to
  )
  const isTaskRangeInvalid = Boolean(
    taskFilters.dueFrom && taskFilters.dueTo && taskFilters.dueFrom > taskFilters.dueTo
  )

  const downloadFile = async (
    scope: 'companies' | 'tasks',
    path: string,
    filename: string,
    ...
    if (scope === 'companies' && isCompanyRangeInvalid) {
      setError('開始日が終了日より後になっています。日付を確認してください。')
      setErrorScope('companies')
      setActiveDownload('')
      showToast('日付の入力を確認してください', 'error')
      return
    }
    if (scope === 'tasks' && isTaskRangeInvalid) {
      setError('開始日が終了日より後になっています。日付を確認してください。')
      setErrorScope('tasks')
      setActiveDownload('')
      showToast('日付の入力を確認してください', 'error')
  ...
  return (
    <div className="space-y-4 ">
      <div>
        <p className="text-sm uppercase  text-slate-400">エクスポート</p>
        <h2 className="text-3xl font-bold text-slate-900">CSVエクスポート</h2>
      </div>

      {error && !errorScope && <ErrorAlert message={error} />}

      <div className="grid gap-6 lg:grid-cols-2">
        <ExportCard
          title="企業エクスポート"
          onDownload={() =>
            downloadFile('companies', apiRoutes.exports.companies(), 'companies.csv', companyFilters)
          }
          isLoading={activeDownload === 'companies'}
          loadingLabel="ダウンロード中..."
          buttonLabel="CSVをダウンロード"
          isDisabled={isCompanyRangeInvalid}
        >
          <DateInput
          ...
          {errorScope === 'companies' && <ErrorAlert message={error} className="sm:col-span-2" />}
          {isCompanyRangeInvalid && (
            <div className="text-xs text-rose-600 sm:col-span-2">
              開始日と終了日を確認してください。
            </div>
          )}
        </ExportCard>
        <ExportCard
          title="タスクエクスポート"
          onDownload={() =>
            downloadFile('tasks', apiRoutes.exports.tasks(), 'tasks.csv', taskFilters)
          }
          isLoading={activeDownload === 'tasks'}
          loadingLabel="ダウンロード中..."
          buttonLabel="CSVをダウンロード"
          isDisabled={isTaskRangeInvalid}
        >
          <DateInput
          ...
          {errorScope === 'tasks' && <ErrorAlert message={error} className="sm:col-span-2" />}
          {isTaskRangeInvalid && (
            <div className="text-xs text-rose-600 sm:col-span-2">
              開始日と終了日を確認してください。
            </div>
